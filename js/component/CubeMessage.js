class CubeMessage extends Component {
	constructor(props) {
		...
		var curVoiceMessage = new CubeVoiceMessage(window.loginInfo.messagePeer);
	...
	sendRecord() {
		this.curVoiceMessage.stopRecord(() => {
			this.isClickAudio = !this.isClickAudio
			document.querySelector('.audioInfo').classList.remove('active')
			this.messageService.sendMessage(window.loginInfo.messagePeer + '', this.curVoiceMessage);
			setTimeout(() => {
